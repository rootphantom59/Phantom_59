...
# ==================================================================
#   PHANTOM - Ultimate Terminal Setup (Interactive Installer)
...
OBJ="$DIR/files"
G="\033[1;32m"; C="\033[1;36m"; R="\033[1;31m"; W="\033[1;37m"; Y="\033[1;33m"; RS="\033[0m"
...
if ! command -v lolcat >/dev/null 2>&1; then
    gem install lolcat >/dev/null 2>&1 || echo -e "${R}[!] Failed to install lolcat. Banner will use plain colors.${RS}"
fi
...
if [ ! -d "$HOME/.oh-my-zsh/plugins/zsh-autosuggestions" ]; then
    git clone --depth=1 https://github.com/zsh-users/zsh-autosuggestions "$HOME/.oh-my-zsh/plugins/zsh-autosuggestions" >/dev/null 2>&1
fi
if [ ! -d "$HOME/.oh-my-zsh/plugins/zsh-syntax-highlighting" ]; then
    git clone --depth=1 https://github.com/zsh-users/zsh-syntax-highlighting.git "$HOME/.oh-my-zsh/plugins/zsh-syntax-highlighting" >/dev/null 2>&1
fi
...
mkdir -p "$FIGLET_DIR" 2>/dev/null || sudo mkdir -p "$FIGLET_DIR"
if $IS_TERMUX; then
    cp "$OBJ/PHANTOM.flf" "$FIGLET_DIR/PHANTOM.flf"
else
    sudo cp "$OBJ/PHANTOM.flf" "$FIGLET_DIR/PHANTOM.flf"
fi

# ---------- 5. Termux properties (colors + extra keys) ----------
if $IS_TERMUX; then
    mkdir -p "$HOME/.termux"
    cp "$OBJ/colors.properties" "$HOME/.termux/colors.properties"
    cp "$OBJ/termux.properties" "$HOME/.termux/termux.properties"
...
cp "$OBJ/help.sh"    "$CONFIG_DIR/bin/help.sh"
cp "$OBJ/bname.sh"   "$CONFIG_DIR/bin/bname.sh"
cp "$OBJ/unstall.sh" "$CONFIG_DIR/bin/unstall.sh"
cp "$OBJ/art.sh"     "$CONFIG_DIR/bin/art.sh"
...
if command -v figlet >/dev/null 2>&1; then
    BANNER=\$(figlet -d "\$FIGLET_DIR" -c -f PHANTOM -w \$W "$banner_text" 2>/dev/null)
else
    BANNER="  $banner_text"
fi
...
printf "\n  \033[1;34mSystem : \033[1;32mOnline       \033[1;34mUser : \033[1;32m$prompt_user\033[0m\n"
printf "  \033[1;34mHost : \033[1;36m$prompt_host\033[0m\n"
printf "  \033[1;34mType : \033[1;33mPxhelp \033[1;37mFor Command List\033[0m\n"
